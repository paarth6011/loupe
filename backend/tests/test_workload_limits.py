...
def test_get_or_create_is_idempotent(db_session):
    first = _get_or_create_workload(db_session, "same", max_workloads=0)
    db_session.flush()
    again = _get_or_create_workload(db_session, "same", max_workloads=0)
    assert first.id == again.id
    count = db_session.query(Workload).filter(Workload.name == "same").count()
    assert count == 1


def test_cardinality_cap_blocks_new_workloads(db_session):
    _get_or_create_workload(db_session, "a", max_workloads=2)
    _get_or_create_workload(db_session, "b", max_workloads=2)
    with pytest.raises(HTTPException) as exc:
        _get_or_create_workload(db_session, "c", max_workloads=2)
    assert exc.value.status_code == 429
    # The cap never blocks an *existing* workload from resolving.
    existing = _get_or_create_workload(db_session, "a", max_workloads=2)
    assert existing.name == "a"


def test_cardinality_cap_disabled_with_zero(db_session):
    for name in "abcdef":
        _get_or_create_workload(db_session, name, max_workloads=0)
    assert db_session.query(Workload).count() == 6


def test_get_or_create_recovers_from_lost_insert_race(db_session, monkeypatch):
    ...
    result = _get_or_create_workload(db_session, "raced", max_workloads=0)
